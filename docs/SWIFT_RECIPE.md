...
| "restored accuracy with RL (GSPO), on-policy distillation and ThinkingCap adapter chunks" | `scripts/opd_restore.py --steps 300`; optional `swiftlab transfer --only mlp --alpha 0.4 --keep 0.2` | OPD = sample from the student, reverse-KL to the frozen base on the student's own tokens, wrong traces weighted 2×. GSPO is not implemented; OPD alone is the cheaper path they hinted at ("secret sauce"). |
